...
exports.Scraper = function() {
    ...
    this.scrapePage = function(document) {
        ...
        card_items.update().eachWait(function(card_item, each_done, each_error) {
            ...
            // done all
            var page_links = document.querySelector('div.pagingControls')
    ...
    this.fetchAndScrapePage = function(page_no, success, failure) {
        ...
        browser.visit(url.format(fetchurl), function(err, browser, status) {
            ...
            success(this.scrapePage(browser.document));
        }.bind(this));
    };
    ...
    this.fullScrape = function(done) {
        ...
        var scrapeRecurse = function(need_to_do_more) {
            if(need_to_do_more) {
                this.fetchAndScrapePage(count, function() {
                    count ++;
                    scrapeRecurse(count);
                }, function() {
                    throw new Error("WTF!");
